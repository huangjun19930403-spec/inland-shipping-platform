...
BUILTIN_DICTS: list[dict[str, Any]] = [
    ...
    {"dict_code": "VESSEL_QUALITY_ISSUE_TYPE", "dict_name": "船舶数据质量问题类型", "items": _items(("MMSI_CONFLICT", "ACTIVE MMSI 冲突"), ("PRIMARY_RELATION_MISSING", "主关系缺失"), ("PROFILE_FIELD_MISSING", "主档字段缺失"), ("OCR_UNCONFIRMED", "OCR 待确认"), ("AIS_UNMATCHED", "AIS 未匹配"), ("POSITION_STALE", "船位过期"))},
    ...
    {"dict_code": "VESSEL_CONFIDENCE_LEVEL", "dict_name": "船舶数据可信等级", "items": [_item("HIGH", "高可信", color="success"), _item("MEDIUM", "中可信", color="warning"), _item("LOW", "低可信", color="danger"), _item("UNKNOWN", "未知", color="info", is_default=True)]},
    ...
    {"dict_code": "VESSEL_ANALYSIS_ANNOTATION_TYPE", "dict_name": "船舶分析标注类型", "items": [_item("DATA_TRUSTED", "数据可信", color="success"), _item("DATA_INSUFFICIENT", "数据不足", color="warning"), _item("SAMPLE_REFERENCEABLE", "样本可参考", color="success"), _item("SAMPLE_NOT_REFERENCEABLE", "样本不宜参考", color="danger"), _item("CONTACT_SUSPECTED_INVALID", "联系人疑似失效", color="warning"), _item("CERTIFICATE_RISK", "证照风险", color="danger"), _item("POSITION_ABNORMAL", "位置异常", color="warning"), _item("TONNAGE_MISMATCH", "吨位不适配", color="warning"), _item("NEEDS_REVIEW", "需复核", color="info", is_default=True)]},
    {"dict_code": "VESSEL_ANALYSIS_SAMPLE_TAG", "dict_name": "船舶分析样本标签", "items": [_item("HIGH_QUALITY_PROFILE", "高质量档案", color="success"), _item("ACTIVE_SAMPLE", "活跃样本", color="primary"), _item("LOW_RISK_SAMPLE", "低风险样本", color="success"), _item("CONTACT_REFERENCEABLE", "联系人可参考", color="warning")]},
    ...
    {"dict_code": "VESSEL_DATA_SOURCE_TYPE", "dict_name": "船舶数据来源层", "items": _items(("VESSEL_PROFILE", "船舶主档"), ("RELATION_LEDGER", "关系账本"), ("CERTIFICATE_LEDGER", "证书账本"), ("QUALITY_ISSUE", "质量问题"), ("ES_REALTIME", "实时 AIS"), ("PROFILE_SUMMARY", "资产摘要"))},
