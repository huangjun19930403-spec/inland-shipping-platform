...
from scripts.seeds.demo.experience.shared import NODE_CODES, ROUTES
...
async def verify() -> list[CheckResult]:
    ...
    async with AsyncSessionLocal() as session:
        count_checks = [
            ("business regions", await _count(session, Region, Region.code.not_like("E2E%")), 6),
            ...
            ("node contacts empty-ok", await _count(session, TransportNodeContact), 0),
            ...
            ("vessel owner periods empty-ok", await _count(session, VesselOwnerPeriod), 0),
            ("vessel current owners empty-ok", await _count(session, VesselOwnerPeriod, VesselOwnerPeriod.is_current.is_(True)), 0),
            ("vessel historical owners empty-ok", await _count(session, VesselOwnerPeriod, VesselOwnerPeriod.is_current.is_(False)), 0),
            ("vessel operator periods empty-ok", await _count(session, VesselOperatorPeriod), 0),
            ("vessel current operators empty-ok", await _count(session, VesselOperatorPeriod, VesselOperatorPeriod.is_current.is_(True)), 0),
            ("vessel contacts", await _count(session, VesselContact), 137),
            ("vessel operator contacts empty-ok", await _count(session, VesselContact, VesselContact.contact_scope_code == "OPERATOR"), 0),
            ...
            ("vessel AIS city snapshot items", await _count(session, VesselAisCitySnapshotItem), 4),
            ("vessel latest positions", await _count(session, VesselLatestPositionSnapshot), 72),
            ("vessel spatial snapshots", await _count(session, VesselSpatialObservationSnapshot), 7),
            ...
            ("vessel quality issues empty-ok", await _count(session, VesselDataQualityIssue), 0),
            ("vessel risk signals empty-ok", await _count(session, VesselRiskSignal), 0),
            ("vessel governance tasks empty-ok", await _count(session, VesselGovernanceTask), 0),
            ("vessel certificate requirement rules empty-ok", await _count(session, VesselCertificateRequirementRule), 0),
            ("vessel recognition diffs empty-ok", await _count(session, VesselRecognitionFieldDiff), 0),
            ("vessel recognition adoptions empty-ok", await _count(session, VesselRecognitionAdoptionRecord), 0),
            ...
            ("vessel navigation constraint evidence", await _count(session, VesselNavigationConstraintEvidence), 18),
            ("vessel blacklist signals empty-ok", await _count(session, VesselBlacklistSignal), 0),
            ("vessel controller evidence empty-ok", await _count(session, VesselControllerEvidence), 0),
            ("vessel affiliation evidence empty-ok", await _count(session, VesselAffiliationEvidence), 0),
            ("vessel certificates empty-ok", await _count(session, VesselCertificate), 0),
            ("vessel change events empty-ok", await _count(session, VesselChangeEvent), 0),
            ...
            ("freight daily facts", await _count(session, FactFreightDaily), 7),
            ("freight city facts", await _count(session, FactFreightCityDaily), 2),
            ("freight flow facts", await _count(session, FactFreightFlowDaily), 2),
            ("freight node facts", await _count(session, FactFreightNodeDaily), 2),
            ("ship city facts", await _count(session, FactShipCityDaily), 2),
        ...
        results.extend(
            [
                _result(
                    ...
                    and len(freight_flow_overview.freight_structure) >= 1
                    and len(freight_flow_overview.freight_corridors) >= 1,
                ...
                _result(
                    "flow ship workbench summary ready",
                    len(ship_flow_overview.ship_summary) >= 4
                    and len(ship_flow_overview.ship_corridors) >= 1
                    and len(ship_flow_overview.ship_flow_details) >= 1,
        ...
        demo_route_code = next(route.code for route in ROUTES if route.key == "TAICANG_WUHU")
        demo_node_id = await session.scalar(select(TransportNode.id).where(TransportNode.code == NODE_CODES["TAICANG"]))
        demo_destination_node_id = await session.scalar(select(TransportNode.id).where(TransportNode.code == NODE_CODES["WUHU"]))
        demo_route_id = await session.scalar(select(ShippingRoute.id).where(ShippingRoute.code == demo_route_code))
        demo_region_id = await session.scalar(
            select(ShippingRoute.origin_region_id).where(ShippingRoute.code == demo_route_code)
        )
        ...
        for context_name, payload in demo_context_cases:
            ...
            ok = response.status_code != "NOT_COMPUTABLE" and response.candidate_count >= 8
            if ok:
        ...
        results.append(_result("raw-level formal freights seeded", raw_freight_count >= 4, f"{raw_freight_count} >= 4"))
