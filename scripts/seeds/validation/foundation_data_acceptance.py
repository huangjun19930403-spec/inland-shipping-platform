...
async def verify() -> list[CheckResult]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        required_counts = {
            ...
            if key != "node_contacts"
        }
        demo_node_count = await session.scalar(
            select(func.count())
            .select_from(TransportNode)
            .where(TransportNode.code.like("NODE_%"))
        )
        counts_ok = all((value or 0) > 0 for value in required_counts.values())
        contacts_ok = (counts["node_contacts"] or 0) > 0 or (demo_node_count or 0) == 0
        results.append(_result("foundation production data exists", counts_ok and contacts_ok, str(counts)))
