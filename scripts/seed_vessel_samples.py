...
async def seed_vessel_samples() -> None:
    async with AsyncSessionLocal() as session:
        ...
        for idx, ship_type_code in enumerate(_type_codes(), start=1):
            ...
            session.add(VesselOwnerPeriod(vessel_profile_id=vessel.id, party_name=OWNER_NAMES[idx % len(OWNER_NAMES)], party_type_code="COMPANY" if idx % 7 else "PERSON", party_relation_type_code="OWNER", certificate_no=f"VESSEL-OWNER-{idx:04d}", mobile_phone=f"18{idx % 10}{(idx * 19):08d}"[:11], address=f"{city.name}港航服务中心", start_date=date(2020, 1, 1), is_current=True, is_primary=True))
            session.add(VesselOperatorPeriod(vessel_profile_id=vessel.id, operator_name=OWNER_NAMES[(idx + 1) % len(OWNER_NAMES)], party_type_code="COMPANY", operator_role_code="OPERATOR", manager_name=OWNER_NAMES[idx % len(OWNER_NAMES)], main_navigation_area_desc=region.short_name or region.name, usual_route_desc=f"{city.name} - {region.short_name or region.name}", contact_phone=f"13{idx % 10}{idx:08d}"[:11], dispatch_contact_name=f"调度{idx:03d}", dispatch_contact_phone=f"15{idx % 10}{(idx * 37):08d}"[:11], risk_level_code="LOW" if idx % 11 else "MEDIUM", start_date=date(2020, 1, 1), is_current=True, is_primary=True, last_active_at=now))
            session.add(VesselContact(vessel_profile_id=vessel.id, contact_name=f"船务联系人{idx:03d}", contact_role_code="DISPATCH_CONTACT", mobile_phone=f"17{idx % 10}{(idx * 29):08d}"[:11], wechat=f"ship{idx:04d}", email=f"ship{idx:04d}@example.local", is_primary=True, is_available=idx % 13 != 0, last_verified_at=now, remark="本地验证预制联系人，可用于船舶资料维护回归"))
            if idx <= 92:
