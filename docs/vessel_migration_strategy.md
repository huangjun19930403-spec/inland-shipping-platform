...
V3 采用当前态干净基线：`alembic/versions/0001_platform_current_schema.py` 是唯一 active Alembic migration，`down_revision = None`。旧 `0001` 到 `0036` 补丁链已退出 active 迁移目录，不再作为本地调试或准生产初始化路径。
...
- active Alembic 目录只允许 `0001_platform_current_schema.py`。
