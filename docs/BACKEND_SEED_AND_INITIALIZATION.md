...
- 船舶样例支撑船舶列表、详情和分布图表。
- 正式货源、微信采集批次、TMS 入站、线索、候选和人工反馈支撑货源采集主链。
- 分析 seed 先写入 `analysis_job_definition`，再从已 seed 的货源、船舶和基础数据聚合生成事实表。
- 货源事实尽量来自真实 `freight` 样例聚合；船舶城市热力和船舶流向在未接入 ES 的本地环境使用确定性 `LOCAL_SAMPLE` 样例源。
- 分析事实数据支撑船舶、货源、区域、流向、运价分析和分析任务页。
...
- 菜单不包含旧入口。
- 主业务数据不包含 `E2E_%` 编码。
- 分析任务定义、运行记录、城市事实、流向事实和 seed 幂等检查通过。
