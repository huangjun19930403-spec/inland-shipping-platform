...
async def list_regions(
    ...
    for item in result["items"]:
        data = RegionResponse.model_validate(item["region"])
        data.waterway_ids = item.get("waterway_ids", [])
        data.city_ids = item.get("city_ids", [])
        items.append(data)
...
async def list_regions_paged(
    ...
    for item in result["items"]:
        ...
        detail.waterway_ids = item.get("waterway_ids", [])
        detail.city_ids = item.get("city_ids", [])
