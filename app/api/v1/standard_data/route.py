...
@router.post("/route/{route_id}/path/{path_id}/segment", summary="向路线方案添加分段")
async def add_path_segment(
    ...
    obj = await service.add_path_segment(
        route_id=route_id,
        path_id=path_id,
        sequence=data.sequence,
...
@router.put("/route/{route_id}/path/{path_id}/segments", summary="批量替换路线方案的所有分段")
async def set_path_segments(
    ...
    obj = await service.set_path_segments(
        route_id=route_id,
        path_id=path_id,
        segments=[s.model_dump(exclude_none=True) for s in data.segments],
    )
    return success(data=ShippingRoutePathResponse.model_validate(obj))


@router.delete(
    "/route/{route_id}/path/{path_id}/segment/{segment_id}",
    summary="删除路线方案中的分段",
)
async def delete_path_segment(
    ...
    await service.delete_path_segment(
        route_id=route_id, path_id=path_id, segment_id=segment_id
    )
    return success(message="删除成功")
