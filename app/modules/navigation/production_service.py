...
STAGE_LABELS = {
    ...
    "WATER_MATCH_READY": "边界待生产",
    "BOUNDARY_CANDIDATE": "边界待生产",
    "BOUNDARY_PUBLISHED": "中心线待生产",
    "CENTERLINE_CANDIDATE": "中心线待发布",
    "CENTERLINE_PUBLISHED": "Graph 待构建",
    "GRAPH_READY": "路径待验证",
    "ROUTE_VERIFIED": "路径已验证",
    "BLOCKED": "生产阻塞",
}
NEXT_ACTIONS = {
    "NO_WATER_MATCH": ("确认水体归属", "/navigation/production/water-matches"),
    "WATER_MATCH_READY": ("生产边界", "/navigation/production/boundaries"),
    "BOUNDARY_CANDIDATE": ("编辑并发布边界", "/navigation/production/boundaries"),
    "BOUNDARY_PUBLISHED": ("导入或绘制中心线", "/navigation/production/centerlines"),
    "CENTERLINE_CANDIDATE": ("发布中心线", "/navigation/production/centerlines"),
    "CENTERLINE_PUBLISHED": ("构建 Graph", "/navigation/production/graphs"),
...
class NavigationProductionService:
    ...
    async def create_osm_import(self, body: NavigationOsmImportRequest) -> NavigationOsmImportResponse:
        ...
        if source_path is None or not source_path.exists():
            return NavigationOsmImportResponse(
                ...
                message="请先放入真实 OSM waterway 数据文件；系统不会生成示例中心线。",
        ...
        return NavigationOsmImportResponse(
            ...
            message=f"已处理真实 OSM waterway GeoJSON：候选 {summary.candidate_count} 条，跳过 {summary.skipped_count} 条。",
            next_path="/navigation/production/centerlines",
        )
    ...
    async def _production_channels(self, channels: list[NavigationChannel]) -> list[NavigationProductionChannelResponse]:
        ...
        current_boundary_counts = await self.workbench._counts_by_channel(
        ...
        approved_centerline_counts = await self.workbench._counts_by_channel(
            NavigationChannelCenterline.channel_id,
            select(NavigationChannelCenterline.channel_id, func.count()).where(
                NavigationChannelCenterline.channel_id.in_(channel_ids),
        ...
        for channel in channels:
            counts = {
                ...
                "boundary": current_boundary_counts.get(channel.id, 0),
                "centerline_candidate": centerline_candidate_counts.get(channel.id, 0),
                "centerline": approved_centerline_counts.get(channel.id, 0),
    ...
    def _stage(self, counts: dict[str, int]) -> tuple[str, list[str]]:
        ...
        if counts["boundary"] <= 0:
            return "WATER_MATCH_READY", ["NO_CURRENT_BOUNDARY"]
        ...
        return "BOUNDARY_PUBLISHED", ["NO_APPROVED_CENTERLINE"]
    ...
    def _steps(self, counts: dict[str, int]) -> list[NavigationProductionStepResponse]:
        return [
            ...
            NavigationProductionStepResponse(
                ...
                status_code="NEED_REVIEW" if counts["candidate_boundary"] else ("READY" if counts["boundary"] else "BLOCKED"),
                count=counts["candidate_boundary"] or counts["boundary"],
                blocker_code="BOUNDARY_CANDIDATE_TO_PUBLISH" if counts["candidate_boundary"] else None,
            ...
            NavigationProductionStepResponse(
                ...
                blocker_code=None if counts["centerline"] else ("CENTERLINE_CANDIDATE_TO_PUBLISH" if counts["centerline_candidate"] else "NO_APPROVED_CENTERLINE"),
                next_path="/navigation/production/centerlines",
            ),
            NavigationProductionStepResponse(
                step_code="GRAPH",
    ...
    def _available_actions(self, channel_id: int, counts: dict[str, int], stage: str) -> list[dict[str, Any]]:
        actions: list[dict[str, Any]] = [
            ...
            },
            {
                ...
                "enabled": counts["boundary"] > 0 or counts["candidate_boundary"] > 0,
                "disabled_reason": None if counts["boundary"] > 0 or counts["candidate_boundary"] > 0 else "NO_BOUNDARY_CONTEXT",
            },
            {
                ...
                "disabled_reason": None if counts["centerline"] > 0 else "NO_APPROVED_CENTERLINE",
            },
            {
    ...
    def _warnings_for(self, row: NavigationProductionChannelResponse) -> list[str]:
        ...
        if row.candidate_boundary_count > 0:
            warnings.append("候选边界来自真实水系归属，只能载入草稿编辑后发布。")
        if row.approved_current_centerline_count <= 0:
            warnings.append("无已发布中心线，Graph 构建和路径生成必须失败。")
        return warnings
