...
class RouteService:
    ...
    async def add_path_segment(
        ...
        await self.get_route_path(route_id, path_id)
        segment = ShippingRoutePathSegment(
            path_id=path_id,
            sequence=sequence,
    ...
    async def set_path_segments(
        self, route_id: int, path_id: int, segments: list
    ) -> ShippingRoutePath:
        """批量替换路线分段（先清空，再重建）"""
        await self.get_route_path(route_id, path_id)
        ...
        for s in segments:
            segment = ShippingRoutePathSegment(
                path_id=path_id,
                sequence=s["sequence"],
                segment_type=s.get("segment_type", "WATERWAY"),
                from_node_id=s.get("from_node_id"),
                to_node_id=s.get("to_node_id"),
                distance_km=s.get("distance_km"),
                estimated_duration_hours=s.get("estimated_duration_hours"),
                description=s.get("description"),
            )
            await self._route.create_path_segment(segment)
        await self._route.save()
        return await self._route.get_path(path_id)

    async def delete_path_segment(
        self, route_id: int, path_id: int, segment_id: int
    ) -> None:
        await self.get_route_path(route_id, path_id)
        deleted = await self._route.delete_path_segment(segment_id)
        if not deleted:
