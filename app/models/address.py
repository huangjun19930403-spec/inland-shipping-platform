...
class NavigationChannelSegment(Base, TimestampMixin):
    ...
    guide_geometry_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    guide_source_type_code: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    guide_quality_code: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    guide_length_m: Mapped[float | None] = mapped_column(Numeric(14, 2), nullable=True)
    guide_bbox_min_lng: Mapped[float | None] = mapped_column(Numeric(24, 15), nullable=True)
    guide_bbox_min_lat: Mapped[float | None] = mapped_column(Numeric(24, 15), nullable=True)
    guide_bbox_max_lng: Mapped[float | None] = mapped_column(Numeric(24, 15), nullable=True)
    guide_bbox_max_lat: Mapped[float | None] = mapped_column(Numeric(24, 15), nullable=True)
    guide_trace_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
