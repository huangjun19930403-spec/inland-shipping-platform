...
class ShipImportBatch(Base, TimestampMixin):
    __tablename__ = "ship_import_batch"
...
class ShipImportRaw(Base):
    __tablename__ = "ship_import_raw"
    __table_args__ = (
        UniqueConstraint("batch_id", "row_no", name="uk_ship_import_raw_row"),
    )

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    batch_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("ship_import_batch.id"), nullable=False, index=True
    )
    row_no: Mapped[int] = mapped_column(Integer, nullable=False)
    raw_payload_json: Mapped[dict] = mapped_column(JSON, nullable=False)
    parse_status_code: Mapped[str] = mapped_column(String(64), nullable=False)
    parse_message: Mapped[str | None] = mapped_column(String(512), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)


class ShipImportRecord(Base):
    __tablename__ = "ship_import_record"
    ...
    raw_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("ship_import_raw.id"), nullable=False, index=True
    )
    ship_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("ship_profile.id"), nullable=True, index=True
    )
    action_type_code: Mapped[str] = mapped_column(String(64), nullable=False)
